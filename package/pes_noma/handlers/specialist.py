...
class UniversalEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        ...
        if hasattr(
            obj, "isoformat"
        ):  # Handles date, time, and other datetime-like objects
            return obj.isoformat()
...
class Specialist:
    ...
    def _summarize_plan(self, plan_data, state_data, current_step_id, workspace=None):
        ...
        if legs_array:
            summary_lines.append(
                "When calling flight search tools (e.g. search_flights_rextur), respect the Rextur contract:"
            )
            summary_lines.append(
                "  • Full multi-city itinerary (2–6 open-jaw / chained legs): ONE call with ALL legs "
                "in `legs`=[...] — native MC fare (same product as Rextur Multitrechos)."
            )
            summary_lines.append(
                "  • Round trip / closed loop only (A→B then B→A, city-aware: GIG/SDU = Rio, GRU/CGH = SP): "
                "ONE call with BOTH legs — coupled RT fare."
            )
            summary_lines.append(
                "  • If native MC / coupled call fails or returns empty, fall back to one-way per leg "
                "(or try-casada on closed pairs). Do not invent splits unless nudged."
            )
            summary_lines.append("Plan flight segments (chronological):")
            for i, leg in enumerate(legs_array):
                summary_lines.append(
                    f"  Segment {i}: {leg['origin']} -> {leg['destination']} on {leg['date']}"
                )
            summary_lines.append("")

            def _city_key(code: str) -> str:
                # Lightweight city grouping for the common BR multi-airport hubs
                # (mirrors backend iata_to_city for the airports we see most).
            ...
            is_coupled_rt = False
            if len(legs_array) == 2:
                a, b = legs_array[0], legs_array[1]
                is_coupled_rt = _city_key(a.get("origin")) == _city_key(
                    b.get("destination")
                ) and _city_key(a.get("destination")) == _city_key(b.get("origin"))
            if is_coupled_rt:
                summary_lines.append(
                    "For search_flights_rextur, set the 'legs' parameter to exactly this array (copy it as-is):"
                )
                summary_lines.append(json.dumps(legs_array))
                summary_lines.append(
                    "The 'legs' array MUST be ordered by ascending date (earliest first)."
                )
                summary_lines.append(
                    "Use the above legs unless the user explicitly asks for something else (e.g. different dates or segments)."
                )
            else:
                summary_lines.append(
                    "For search_flights_rextur, call the tool ONCE PER segment below — each call's `legs` must contain ONLY that single segment."
                )
                summary_lines.append(
                    "Search every named segment in the SAME turn; present only the FIRST leg's options (remaining legs queue as pending)."
                )
                for i, leg in enumerate(legs_array):
                    summary_lines.append(f"  Call {i + 1} legs={json.dumps([leg])}")
            summary_lines.append("")
        ...
        for step in plan_data["steps"]:
            ...
            summary_lines.append(
                f"{symbol} Step {step_id}: {step.get('title', f'Step {step_id}')} [{status}]{marker}"
            )
            ...
            for entry in action_log:
                ...
                name = (
                    ...
                    if tool != "*"
                        ...
                        entry.get("message", "User interaction")[:50]
                        or "User interaction"
                    )
                )
                if STATUS_RANK.get(code, 0) >= STATUS_RANK.get(
                    str((tool_states.get(tool) or {}).get("code", "0")), 0
                ):
    ...
    def consent_form(self, payload):
        ...
        consent = {
            ...
            "message": {"role": "assistant", "content": consent_msg},
        }
    ...
    def interpret(self, no_tools=False, tool_result=False):
        ...
        try:
            ...
            belief_str = "Current beliefs: " + self.AGU.string_from_object(
                current_beliefs
            )
            ...
            message_filter = {
                "param": "_next",
                "begins_with": f"irn:c_id:{continuity['plan_id']}:{continuity['plan_step']}",
            }
            ...
            if not message_list["output"]:
                if isinstance(current_beliefs, dict):
                    inputs = ", ".join(
                        [f"{k}: {v}" for k, v in current_beliefs.items()]
                    )
                else:
            ...
            for a in list_actions:
                if a["key"] == self._get_context().current_action:
                    ...
                    if (
                        "tools_reference" in a
                        and a["tools_reference"]
                        and a["tools_reference"] not in ["_", "-", "."]
                    ):
            ...
            messages += [
                ...
                {
                    "role": "system",
                    "content": pm.get_optimal_path_instruction(),
            ...
            try:
                ...
                if workspace and plan_id and plan_id in workspace.get("plan", {}):
                    ...
                    plan_summary = self._summarize_plan(
                        plan_data, state_data, current_step_id, workspace
                    )
            ...
            if action_tools and not no_tools:
                messages.append(
                    {
                        "role": "system",
                        "content": f"In case you need them, the following tools are recommended to execute this action: {json.dumps(action_tools)}",
                    }
                )
                ...
                for t in list_tools_raw:
                    if t.get("key") in approved_tools:
                        ...
                        try:
                            ...
                            _logger_specialist.warning(
                                "invalid json tool input tool=%s",
                                t.get("key", "unknown"),
                            )
                            tool_input = []
                        ...
                        if isinstance(tool_input, list):
                            for param in tool_input:
                                ...
                                if param.get("type") == "array" and (
                                    param.get("elements") or param.get("items")
                                ):
                                    items_schema = param.get("elements") or param.get(
                                        "items"
                                    )
                                    prop = {
                                        "type": "array",
            ...
            if "role" in validated_result:
                if (
                    validated_result.get("tool_calls")
                    and validated_result.get("role") == "assistant"
                ):
                    # This is the LLM asking for a tool to be executed.
                    selected_tool = validated_result["tool_calls"][0]["function"][
                        "name"
                    ]
                    _logger_specialist.info("tool call selected=%s", selected_tool)

                    if (
                        continuity["tool_step"] == "3" or continuity["tool_step"] == "4"
                    ) and continuity["action_step"] == selected_tool:
                        ...
                        c_id = (
                            f"{c_id_pre}:{selected_tool}:{tool_step}:{consent['nonce']}"
                        )
                        self.AGU.save_chat(
                            consent["message"], msg_type="consent", next=c_id
                        )

                        validated_result = consent[
                            "message"
                        ]  # Replacing original message with consent message.
                    ...
                    if tool_result == "tool_error":
                        ...
                        if tool_result == "fresh_results":
                            ...
                            _logger_specialist.info(
                                "message to user: %.80s",
                                str(validated_result.get("content", "")),
                            )
                            nonce = random.randint(100000, 999999)
            ...
            return {"success": False, "action": action, "input": "", "output": str(e)}
    ...
    def act(self, command):
        ...
        try:
            ...
            if len(parts) < 2 or len(parts) > 3:
                ...
                _logger_specialist.error(
                    "invalid_handler_route | tool=%s | route=%s",
                    tool_name,
                    handler_route if "handler_route" in dir() else "",
                )
                self.AGU.print_chat(error_msg, "error")
                raise ValueError(error_msg)
            ...
            response_clean = {
                k: v for k, v in response.items() if k not in ["stack", "output"]
            }
            response_clean["output_size"] = len(str(response.get("output", "")))
            _logger_specialist.info(
                "tool=%s returned success=%s details=%s",
                tool_name,
            ...
            self.AGU.mutate_workspace(
                {"cache": {index: value}, "action_log": log_entry},
    ...
    def check(self, command):
        ...
        try:
            ...
            response = self.SHC.handler_check(
                portfolio, org, parts[0], parts[1], params
            )
    ...
    def verify(self, action):
        ...
        try:
            for a in self._get_context().list_actions:
                if a["key"] == self._get_context().current_action:
                    if "verification" not in a:
                        raise Exception(
                            f"No verification handler found for this action: {a.get('key', 'N/A')}"
                        )
            ...
            if len(parts) != 2:
                error_msg = (
                    f"❌ {verification_handler} is not a valid verification tool."
                )
                _logger_verify.error(
                    "invalid_verification_route | handler=%s", verification_handler
                )
                self.AGU.print_chat(error_msg, "error")
                raise ValueError(error_msg)
            ...
            _logger_verify.info(
                "verify_called | action=%s | handler=%s", action, verification_handler
            )

            if response["success"]:
                msg = "Verification OK. Step Completed."
                tool_step = "5"
                continuity = self._get_context().continuity
                log_entry = {
                    "plan_id": continuity["plan_id"],
                ...
                msg = "Step has not been completed yet. Continue the loop"
                continuity = self._get_context().continuity
                log_entry = {
                    "plan_id": continuity["plan_id"],
                    "plan_step": continuity["plan_step"],
            ...
            verification_handler_str = (
                verification_handler if verification_handler else "unknown"
            )
    ...
    def run(self, payload):
        ...
        _logger_specialist.info(
            "specialist started action=%s step_id=%s",
            payload.get("action"),
            payload.get("step_id"),
        )
        ...
        try:
            ...
            context.continuity = payload.get(
                "continuity", {}
            )  # plan_id, plan_step, action_step, tool_id
            ...
            if not context.current_action:
                return {
                    ...
                    "action": action,
                    "input": payload,
                    "output": "No action specified in step",
                }
            ...
            while loops < loop_limit:
                ...
                if (
                    "tool_calls" in response_1["output"]
                    and response_1["output"]["tool_calls"]
                ):
                    # Tool need Execution
                    ...
                    if response_2c["success"]:
                        ...
                        self.AGU.mutate_workspace({"action_log": log_entry})
                        print(
                            f"[SPECIALIST] loop_end {loops}/{loop_limit} | verification=completed"
                        )

                elif (
                    "tool_calls" not in response_1["output"]
                    or not response_1["output"]["tool_calls"]
                ):
                    ...
                    print(
                        f"[SPECIALIST] loop_end {loops}/{loop_limit} | status=awaiting"
                    )
            ...
            self.AGU.print_chat(
                "🤖⚠️  Can you re-formulate your request please?", "text"
            )
            return {
                "success": True,
                "action": action,
                "input": payload,
                "output": response_3["output"],
                "stack": results,
            }
            ...
            return {
                "success": False,
                "action": action,
                "output": f"Run failed. Error:{str(e)}",
                "stack": results,
            }
